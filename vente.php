    ...
    switch ($_POST['action']) {
        case 'search_product':
            ...
            foreach ($products as &$product) {
                $product['stock_disponible'] = max(0, $product['stock_disponible'] - $product['quantite_reservee']);
            }
        ...
        case 'get_product':
            ...
            if ($product) {
                ...
                $product['stock_disponible'] = max(0, $product['stock_actuel'] - $product['quantite_reservee']);
            }
        ...
        case 'verify_stock':
            // Nouvelle action pour vérifier le stock en temps réel
            ...
            foreach ($items as $item) {
                ...
                if ($stock) {
                    $disponible = $stock['stock_actuel'] - $stock['quantite_reservee'];
                    
                    if ($disponible < $requestedQty) {
                        $stockIssues[] = [
                            'id' => $productId,
                            'nom' => $stock['nom_produit'],
                            'demande' => $requestedQty,
        ...
        case 'validate_sale':
            try {
                ...
                // ÉTAPE 1: VÉRIFICATION COMPLÈTE DU STOCK AVANT TOUTE OPÉRATION
                $stockVerification = [];
                foreach ($items as $item) {
                    $productId = intval($item['id']);
                    ...
                    if (!$stock) {
                        throw new Exception("Produit ID $productId introuvable ou inactif");
                    }
                    ...
                    if ($disponible < $quantity) {
                        throw new Exception(
                            "Stock insuffisant pour '{$stock['nom_produit']}' : " .
                            "Disponible: $disponible, Demandé: $quantity"
                        );
                    }
                    
                    $stockVerification[$productId] = [
                        'nom' => $stock['nom_produit'],
                ...
                foreach ($items as $item) {
                    ...
                    // CORRECTION CRITIQUE: Ne plus toucher à quantite_disponible
                    // Mettre à jour UNIQUEMENT quantite_actuelle
                    $result = $db->query("
                        UPDATE stocks 
                    ...
                    if ($result === false) {
                        throw new Exception("Erreur lors de la mise à jour du stock pour le produit ID $productId");
                    }
                ...
                // ÉTAPE 4: Enregistrer dans la trésorerie
                $db->query("
    ...
    <script>
                ...
                html += `
                    <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-400">Stock max: ${item.maxStock}</p>
                        </div>
        ...
        function updateQuantity(index, change) {
            ...
            if (newQuantity > item.maxStock) {
                alert(`Stock insuffisant\nMaximum disponible: ${item.maxStock}`);
                return;
            }
        ...
        async function processSale() {
            ...
            processBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Vérification du stock...';
            disableCartInteractions(true);
            
            try {
                // ÉTAPE 1: Vérifier le stock en temps réel avant la vente
                const stockFormData = new FormData();
                stockFormData.append('action', 'verify_stock');
                stockFormData.append('items', JSON.stringify(cart));
                
                const stockResponse = await fetch('vente.php', {
                    method: 'POST',
                    body: stockFormData
                });
                ...
                if (!stockResult.success) {
                    ...
                    stockResult.issues.forEach(issue => {
                        errorMsg += `• ${issue.nom}\n  Demandé: ${issue.demande}, Disponible: ${issue.disponible}\n\n`;
                    });
                    alert(errorMsg);
                ...
                // ÉTAPE 2: Procéder à la vente
                processBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Traitement de la vente...';
                ...
                const response = await fetch('vente.php', {
                    method: 'POST',
                    body: formData
                });
        ...
        // Fonction pour désactiver/activer les interactions du panier
        ...
        document.addEventListener('keydown', function(e) {
            // Éviter les raccourcis pendant le traitement
        ...
        // Empêcher la soumission multiple via l'événement beforeunload
